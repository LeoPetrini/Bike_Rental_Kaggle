...
def load_files(TRAIN_FILE, TEST_FILE):
    ...
    tr_te['ix_train'] = np.concatenate((np.ones(len(train)),np.zeros(len(test))))
...
def replace_holiday(df, year, month, day, replace):  
    if replace == 'to_holiday':
        df.loc[(df.month == month) & (df.day == day) & (df.year == year), 'holiday'] = 1
        df.loc[(df.month == month) & (df.day == day) & (df.year == year), 'workingday'] = 0
    elif replace == 'to_work':
        df.loc[(df.month == month) & (df.day == day) & (df.year == year), 'holiday'] = 0
        df.loc[(df.month == month) & (df.day == day) & (df.year == year), 'workingday'] = 1
    else:
...
def create_school_holiday(df, school_col, month, day, year):
    df.loc[(df.month == month) & (df.day == day) & (df.year == year), school_col] = 1      
    return df
...
def xgb_tuner(tr, n_models, x_vars, verbose=False):
    ...
    for i in range(n_models):
        ...
        for fold_day in range(len(folds)):
            train_fold = tr[eval(" | ".join(["(tr['{0}'] == {1})".format('day', cond) for cond in folds[fold_day]]))]
            val_fold = tr[eval(" & ".join(["(tr['{0}'] != {1})".format('day', cond) for cond in folds[fold_day]]))]

            d_train_r = xgb.DMatrix(train_fold[x_vars].values, label=np.log(train_fold['registered']+shift_reg))
            d_valid_r = xgb.DMatrix(val_fold[x_vars].values, label=np.log(val_fold['registered']+shift_reg))
            watchlist_r = [(d_train_r, 'train'), (d_valid_r, 'eval')]

            d_train_c = xgb.DMatrix(train_fold[x_vars].values, label=np.log(train_fold['casual']+shift_ca))
            d_valid_c = xgb.DMatrix(val_fold[x_vars].values, label=np.log(val_fold['casual']+shift_ca))
            ...
            predictions[eval(" & ".join(["(tr['{0}'] != {1})".format('day', cond) for cond in folds[fold_day]]))] = y_pred_comb
...
def lgb_tuner(tr, n_models, x_vars, verbose=False):
    ...
    for i in range(n_models):
        ...
        for fold_day in range(len(folds)):
            ...
            lgb_train_r = lgb.Dataset(train_fold[x_vars], np.log(train_fold['registered']+shift_reg))
            lgb_eval_r = lgb.Dataset(val_fold[x_vars], np.log(val_fold['registered']+shift_reg))

            lgb_train_ca = lgb.Dataset(train_fold[x_vars], np.log(train_fold['casual']+shift_reg))
            lgb_eval_ca = lgb.Dataset(val_fold[x_vars], np.log(val_fold['casual']+shift_reg))
            ...
            y_pred_r = np.exp(model_r.predict(val_fold[x_vars], num_iteration=model_r.best_iteration)) - shift_reg
            y_pred_c = np.exp(model_c.predict(val_fold[x_vars], num_iteration=model_c.best_iteration)) - shift_ca
...
def xgb_predict_test(tr, te, x_vars, params, r_ix, ca_ix, shift_ca=1, shift_reg=1):
    ...
    d_train_r = xgb.DMatrix(tr[x_vars].values, label=np.log(tr['registered']+shift_reg))
    d_train_c = xgb.DMatrix(tr[x_vars].values, label=np.log(tr['casual']+shift_ca))
